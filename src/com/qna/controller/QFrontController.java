...
public class QFrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		// getRequestURI() : "/프로젝트명/파일명(*.so)" 라는 문자열을 반환해 주는 메서드.
		 ...
		 * - 보통은 파일에 저장을 하여 사용을 함. 파일 이름을 *.properties라는 이름으로 끝나게 함.
		 * - FileInputStream 클래스에 Properties 파일을 인자로 넣어서 그 스트림으로부터 파일을 가져올 때 많이 사용을 함. 인자로 들어온 Properties 파일을 읽게 됨.
		...
		if(value.substring(0, 7).equals("execute")) {
			 ...
			 * 
			 * - newInstance() 메서드는 기본생성자를 호출해서 객체를 생성하기 때문에 반드시 클래스에 기본생성자가 존재해야 함.
			 *   예외가 발생할 수 있는데 해당 클래스가 추상클래스이거나 인터페이스일 경우 발생하고, 또 하나의 예외는 클래스의 생성자가 접근제한자로 인해 접근할 수 없는 경우에 발생을 함. 
			 *   따라서 예외 처리를 해 주어야 함.
			 *   
			 * - 반환 타입은 Object 타입이므로 맞게 형변환을 해 주면 되지만, 클래스의 타입을 모르는 상태이므로 형변환을 해 줄 수가 없음.
			 *   이러한 문제를 해결하기 위해서 인터페이스를 사용하는 것임.
			 *   
			 * - Class.forName(class 이름)은 파라미터로 받은 class 이름에 해당하는 클래스를 로딩한 후에, 그 클래스에 해당하는 인스턴스를 리턴해 줌.
			 *   newInstance() 메서드는 로딩한 클래스의 객체를 생성하는 메서드이고, 이렇게 생성된 객체를 동적 객체 생성이라고 함.
			 */
		...
		} else { // value에 들어온 값이 "execute"가 아닌 경우
			// view page로 이동
